 ...
 */
class photostreamUploadAction extends CAction {

	//Настя: идентификатор альбома
	public $idAlbumAttribute = "id_album";
    ...
    protected function handleUploading()
    {
		...
		$model->id_album = $_POST['XUploadForm']['id_album'];
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            if ($model->validate()) {
                ...
                if ($returnValue === true) {
                	//добавляем фотографии в базу, $model->id_album
                	
                	$photo = new SPhotostream;
					$photo->date = date('d.m.Y',time());
					$photo->id_album = $model->id_album;
					$photo->image_url = $this->getFileUrl($model->{$this->fileNameAttribute});
					
					if(!$photo->save()) {
						echo json_encode(array(array("error" => "Can't save photo! ",)));
						return false;
					}
            ...
            throw new CHttpException(500, "Ошибка загрузки файла.");
			return false;
        }
    }
